...
The active CNN (`segformer_b0_run3`, see `spec_v005_segformer_train.md`) predicts 7 classes:
...
### Known Mismatch / Technical Debt (task07)

This section and §14 below were originally written against the retired 5-class `background/wall/opening/room/icon` scheme, where `OpeningPrimitive` existed specifically to hold one generic class before heuristic door/window classification. Run3 already predicts `window` and `door_arc`/`door_leaf`/`door_origin` as separate evidence, which makes that heuristic split unnecessary for run3 output — but the primitive set and §14's heuristic-classification guidance have **not** been redesigned to consume the new classes directly. This document is left as historical/in-progress design intent; do not treat §14 as still accurate for run3. A follow-up spec revision is required before the primitive layer is considered aligned with run3.
...
## 14. Door vs Window Concern (stale — see §3 "Known Mismatch / Technical Debt")

This section describes the retired 5-class CNN, not the active `segformer_b0_run3`. Run3 already separates doors and windows into distinct classes, so the heuristic split described below is no longer needed for run3 output. Kept for historical context only.

The previous CNN did not separate doors and windows. Both were included in the `opening` class.
