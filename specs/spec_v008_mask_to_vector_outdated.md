...
The implemented pipeline below (`run_mask_to_vector.py`, `opening_classification.py`, and the four-final-class `floor/wall/opening/icon` SVG output added in task06, §20) still operates on the **retired 5-class** CNN output (`background/wall/opening/room/icon`). The active CNN, `segformer_b0_run3`, predicts 7 classes instead — `window`, `door_arc`, `door_leaf`, and `door_origin` as distinct evidence in place of one heuristically-split `opening`, and `floor` in place of `room`. **This stage has not been updated to consume run3 output.** The heuristic door/window classifier in §4 and §6 Step 6 is now redundant for run3 (the CNN already separates them), and the door-rendering logic added in task05 (§19, 3-component origin/opening/arc symbol) does not match run3's `door_arc`/`door_leaf`/`door_origin` semantics. `configs/vectorization_v008.yaml` also still points its `input.preview_dir` default at the now-deleted `runs/segformer_b0/previews/epoch_030` and uses the old 5-class `classes:` map (see §9). A follow-up spec/code change is required before v008 can correctly vectorize `segformer_b0_run3` output — do not assume this stage is already aligned.

### Input Classes (retired 5-class scheme — what the current implementation still expects)
...
### Active CNN Output (run3, 7 classes — not yet consumed by this stage)
...
**Stale as of task07:** the real file's `input.preview_dir` currently defaults to `runs/segformer_b0/previews/epoch_030`, which was deleted in task07's cleanup (old run1 preview images, see `spec_v005_segformer_train.md`/cleanup history). The `classes:` block below also still reflects the retired 5-class scheme, not run3's 7 classes. Both need to be updated as part of the run3 vectorization follow-up (§3 "Known Mismatch / Technical Debt").

Example fields (as originally written for the 5-class pipeline):
