...
def get_given_layers_random_region(targets, graphs):
    ...
    for bs_i in range(len(targets)):
        ...
        while sum(sampled_points.values()) < sampled_amount:
            ...
            all_sampled_points_adjs.discard((-1, -1))  # use discard: degree-4 nodes have no (-1,-1) slot
            all_sampled_points_adjs = list(all_sampled_points_adjs.difference(all_sampled_points))
            if not all_sampled_points_adjs:
                break  # disconnected graph: frontier exhausted before sampled_amount reached
        ...
        if sum(sampled_points_i.values()) == 0:
            ...
            for unnormalized_point in unnormalized_points:
                for ind, every_point in enumerate(targets_i['unnormalized_points']):
                    ...
                    # Use exact match + break: coordinates are integers so 2-px fuzzy
                    # tolerance is unnecessary and causes duplicates when two nodes sit
                    # within 2 px of each other, making len(indices) > len(points).
                    if every_point[0] == unnormalized_point[0] and every_point[1] == unnormalized_point[1]:
