...
class Backbone(nn.Module):
    def __init__(self, name):
        # pretrained=False: this Backbone's weights get overwritten immediately by
        # load_state_dict() from the SizheHu checkpoint, so there's no need to download
        # ImageNet-pretrained ResNet-50 weights first (and the deprecated `pretrained=`
        # kwarg is on its way out in modern torchvision; kept for now since it still works).
        backbone = getattr(torchvision.models, name)(pretrained=False, norm_layer=FrozenBatchNorm2d)
        super().__init__()
